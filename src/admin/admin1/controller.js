...
exports.admin = async (req, res) => {
    try {
        ...
        if (req.cookies?.accessToken) {
            ...
            if (user) {
                req.user = user;
                let data = { order: 10, user: 10, saloon: 10, FindRequist: 10, Findservice: 10, Findpackeges: 10 }
                req.flash("success", "welcome back to Dashboard");
                res.render("users/dashboard", { user, data, title: "Dashboard" });
            } else {
...
exports.loginData = async (req, res) => {
    try {
        ...
        if (!user) {
            req.flash("error", "wroung email/password details");
            return res.redirect("/");
        };
...
exports.AdminlogOut = async (req, res) => {
    try {
        ...
        res.clearCookie("accessToken", 'token', { expires: new Date(0) }).redirect("/");
    } catch (error) {
        req.flash("error", error.message);
        console.log(error);
    };
